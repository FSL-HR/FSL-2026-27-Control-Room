...
## 2026-07-20 — v8: three new user rules (Cowichan hosting, flight parity, 16U AA piggyback) + 5h LOCAL-tier fix
- Engine (index.html): 4 new hard rules — 'Cowichan hosts at home' (island home games never at LM venues), 'Cowichan showcase slate' (3+ island games in a div-weekend ⇒ resident island team plays 4), '16U AA Cowichan piggyback' (16U AA at Cowichan only when Ravens 12U/14U have games there that weekend), 'Flight parity' (per division, Aeros=Leafs and Glaciers=BVN flight-trip counts; skipped on scoped calls). Plus the LOCAL-tier >5h driveDests fix (Calgary↔Lloyd now caught).
- v7 under new engine: 34 hard (17 5h-drive, 6 C1, 2 C2, 5 C3, 4 parity) → v8: 3 residual, toSource 157→194→182 after claw-back sweep (86 gated moves + 2 home-flips + 2 participant exchanges).
- Residuals (all proven structurally unsatisfiable, user decision needed): C3-Feb 19 (VHA's 5-game season-closing island showcase — Ravens 12U at Edmonton, 14U has no legal opponent, no VHA capacity elsewhere), 16U AAA parity G1/BVN2 (BVN's Nov 13 LM trip pinned by TC/LL flight minimums; G can't gain a flight — VA's Jan 08 Calgary flight comprises exactly the G/BVN games), 19U parity G1/BVN0 (G's Feb 12 LM trip pinned by SP's flight minimum; BVN can't reach any FLY destination legally).
- Lessons: one-location is strict (no island/LM ferry split same weekend); PCN NOT exempt from flight minimums (only reciprocal); Nov 20 blackout covers 16U AAA too; rivalry cap VHA-LLAA 2/weekend bites on consolidation; gate compares msg strings so count-changing partial fixes must be batched with their completion; CLAW sweep (To-Source → same-cluster real spare, engine-gated, ts-must-drop) recovered 12.
- Deliverable FSL_schedule_2026-07-20_FINAL_v8.json (repo + Downloads), fresh-load verified: 768 games, 3 hard, ts 182, Aeros-Nats 14U ×4, doubles 254 all ≥165. Replay log: rebuild/v8_moves_checkpoint.js.

## 2026-07-21 — v9: Purcell trip minimums + play-the-host rule
- Two new engine rules (index.html): (1) "Purcell trip minimums" — Purcell no longer exempt; far trips need 4 (19U 3), Lower Mainland allowance 16U 3 / 19U 2 (user's Delta rule). (2) "Play the host" — a team visiting another region must play a host-region team >=1× that weekend; exempts Midway/Hardisty neutrals, island<->LM ferry, short LOCAL hops, and (user decision 2026-07-21) is SATISFIED once the host is "full": a visitor is exempt when hosts present already play min(#visitors, host game count) distinct visitors (overflow at showcases unavoidable).
- v8 under new rules = 45 hard. Fixed 38 → 7 residual. Method: HOSTFIXW (within-weekend host re-pair, swap visitor-opponent with host-opponent) cleared 11; HOSTFIXG (generalized cross-weekend exchange) cleared a few; ADDP (add sourced Purcell games to reach minimums, per user "add ice" choice) added 6 games (games 768→774, ts 182→184). Aeros-Nats 14U ×4 preserved, doubles all >=165.
- 4 NEW residuals (all proven infeasible without restructuring showcases — user to accept or direct): (a) Angels 14U AA Nov 06 Aberdeen host — the only re-pair partners (Calgary Glaciers-AA, Bow Valley-AA) are same-city rivals barred from playing each other away, so Sask would need 6 games (cap 4). (b) Sask Phantoms-AA 16U AA Jan 08 Winnipeg host — same rivalry lock, Winnipeg fixed-count. (c) Purcell-Green 19U Feb 19 Edmonton min 2/3 and (d) Purcell-Gold 19U Feb 19 Aberdeen min 2/3 — both weekends packed at the 19U cap of 3, no opponent with room to add a 3rd game. Plus the 3 pre-existing v8 residuals (1 Cowichan piggyback Feb 19, 2 flight-parity).
- Key insight for the host rule: re-pair WITHIN the weekend (swap the visitor's opponent with the host's opponent) keeps venue counts fixed (essential for Winnipeg) and dodges travel/consec; the generalized cross-weekend version is rivalry-locked whenever the only free partners are same-cluster rivals.
- Deliverable FSL_schedule_2026-07-21_FINAL_v9.json (repo + Downloads), fresh-load verified: 774 games, 7 hard, ts 184.

## 2026-07-21 — v9 corrected to 768 games (user: no added games)
- User rejected the +6 sourced Purcell games (wanted total to stay 768). Removed the 6 ADDP additions (Purcell v Sask ×2 Sep11 Ab, Purcell v Aeros Nov13 LM, Purcell v Sask Oct23 Ab, Purcell v Sask Feb12 Ab, Purcell-Green v Glaciers Feb19 Edm) via node — matched on home=/^Purcell/ + day===null + specific weekend/away, asserted exactly 6. Back to 768 games, ts 182.
- Purcell trip-minimum shortfalls (Sep11=2, Oct23=3, Feb12=3 Aberdeen; Nov13=2 LM; both 19U Feb19) now stand as ACCEPTED residuals rather than being filled. Total residuals 12 (6 Purcell-min + 3 play-the-host + 1 Cowichan piggyback + 2 flight-parity), ALL acknowledged via conflictAck in the file (fresh-load shows 0 unacknowledged).
- GOTCHA: the /test/ harness persists slotAdds/conflictAck to localStorage, so sequential page reloads contaminate capacity/ack state — a stale load falsely showed a Canmore Oct09 capFri=1 over-capacity (2 games). A CLEAN localStorage load (localStorage.clear() then navigate) shows the true capFri=2 and no conflict. ALWAYS verify final deliverables with a cleared-localStorage load. Confirmed clean: 768 games, capFri=2, 12 acknowledged residuals, no ice-capacity issue.
- Deliverable FSL_schedule_2026-07-21_FINAL_v9.json (repo + Downloads) = 768 games, 12 accepted+acknowledged residuals, Abbotsford grants kept.
